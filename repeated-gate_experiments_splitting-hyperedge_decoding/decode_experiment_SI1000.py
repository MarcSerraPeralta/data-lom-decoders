...
DISTANCES = [3, 5, 7]
PROBS = [1e-5, 2e-5, 4e-5, 5e-5, 1e-4, 2e-4, 5e-4, 
            ...
            for distance in DISTANCES:
                ...
                for prob in PROBS:
                    ...
                    if get_num_qubits(experiment_name) == 2:
                        # add product Z1*Z2
                        new_log = experiment[-1].targets_copy() + experiment[-2].targets_copy()
                        experiment.append(stim.CircuitInstruction("OBSERVABLE_INCLUDE", targets=new_log, gate_args=[2]))

                    if frame != "gate-independent":
                        dem = experiment.detector_error_model(allow_gauge_detectors=True, decompose_errors=True, ignore_decomposition_failures=True)
                    else:
                        dem = decompose_dem(experiment.detector_error_model(allow_gauge_detectors=True), ignore_logical_error=True)
                    decoder = DECODER(dem)
                    ...
                    sample_failures(
                        ...
                        extra_metrics=extra_metrics_2q if get_num_qubits(experiment_name) == 2 else no_extra_metrics,
